...
export const WeeklyGoal = pgTable("WeeklyGoal", {
  id: serial("id").notNull().primaryKey(),
  userId: varchar("user_id", { length: 255 })
    .notNull()
    .references(() => Users.id),
  goal: integer("number").notNull().default(10),
  progress: integer("number").notNull().default(0),
});
...
export const weeklyGoalRealtions = relations(WeeklyGoal, ({ one }) => ({
  user: one(Users, {
    fields: [WeeklyGoal.userId],
    references: [Users.id],
  }),
}));
